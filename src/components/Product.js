	...
	return (
		<div
			className="flex flex-col items-center justify-between w-full gap-3 p-4 rounded-xl 
                ...
                md:hover:scale-[1.10] transition ease-in"
		>
			<div>
				<p className="black font-semibold text-lg text-left truncate w-40 mt-1">
					{post.title}
				</p>
			</div>
			<div>
				<p className="w-40 text-gray-400 font-semibold text-[10px] text-left">
					{post.description.split(' ').slice(0, 10).join(' ') + '...'}
				</p>
			</div>
			<div className="h-[150px] w-28">
				<img src={post.images[0]} className="h-full w-full" alt="" />
			</div>
